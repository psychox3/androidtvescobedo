...
def first_menu():
        if setting('warning') == "true":
                warning_dialog(title="DiskoKosmiko.mx",text="OCon este Addon accedes a todo el contenido disponible en DiskoKosmiko.mx")
                setSetting('warning',value="false")
        
        myuser=setting('diskokosmiko-username')
        ...
        addDirectoryItem("Buscar", 'search', 'movies.png', 'DefaultMovies.png')
        endDirectory()
...
def go_to_user(query=None):
        ...
        url='%s/%s' % (DiskoKosmikoURL,query)
        open_folder(url)    

def search(query=None):
        types=['Todos los archivos','Video','Imagenes','Musica','Documentos','Archivos','Programas']
        params=['','Video','Image','Music','Document','Archive','Application']
        index=dialog.select('DiskoKosmiko', types)
        if index > -1:
                ...
                show_items(list_items('%s%s' % (DiskoKosmikoURL, SearchParam),query=query,content_type=params[index]))
                endDirectory()
...
def list_folders(url,query=None,result=None):
        try:
                ...
                for indiv in items:
                        ...
                        pageurl = DiskoKosmikoURL + requester.parseDOM(indiv, 'a', attrs = {'class': 'name'}, ret = 'href')[0]
...
def list_items(url,query=None,result=None,content_type=None):
        ...
        for indiv in items:
                ...
                pageurl = DiskoKosmikoURL + requester.parseDOM(requester.parseDOM(indiv, 'div', attrs = {'class': 'name'}), 'a', ret = 'href')[0]
...
def resolve_url(url,play=False):
        ...
        formurl = DiskoKosmikoURL + requester.parseDOM(result, 'form', ret='action', attrs = {'class': 'download_form'})[0]
        headers={'Accept':'*/*','Accept-Encoding':'gzip,deflate','Connection':'keep-alive','X-Requested-With':'XMLHttpRequest'}
...
def play_url(url,name='DiskoKosmiko File',thumb=None,original_url='http://www.example.com/foobar.mp4',original_filename='foobar.mp4'):
