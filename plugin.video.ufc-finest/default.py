...
baseurl         = 'http://dkrepo.netai.net/UltimateUfc/ufcmain.xml'
...
messagetext     = 'http://dkrepo.netai.net/UltimateUfc/info.xml'
...
def GetMenu():
        ...
        addDir('[B][COLOR white]Search[/COLOR] [COLOR red]UFC Finest[/COLOR][/B]',url,5,searchicon,fanarts)		
...
def SCRAPE_MAMAHD(url):
	...
	try:
		...
		dialog.ok("[COLOR white]UFC[/COLOR] [COLOR red]Finest[/COLOR]", "[COLOR white]Sorry, There are no live links in this category at present, please try again later[/COLOR]")
		quit()

	
def SEARCH():
	keyb = xbmc.Keyboard('', '[B][COLOR white]Search[/COLOR] [COLOR red]UFC Finest[/COLOR][/B]')
	keyb.doModal()
	if (keyb.isConfirmed()):
		searchterm=keyb.getText()
		searchterm=searchterm.upper()
	else:quit()
	link=open_url('http://dkrepo.netai.net/UltimateUfc/search.xml')
